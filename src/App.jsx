...
export default function App() {
  ...
  const addTodo = () => {
    ...
    if (trimmedInput && trimmedInput.length <= 500) {
      setTodos([...todos, { id: Date.now(), text: trimmedInput, completed: false, priority, category: trimmedCategory }])
      setInput('')
  ...
  const clearCompleted = () => {
    if (window.confirm(`Delete ${completedCount} completed todo${completedCount !== 1 ? 's' : ''}?`)) {
    ...
    <div className={`app ${darkMode ? 'dark-mode' : 'light-mode'}`}>
          ...
          <div className="category-filters">
            ...
            {allCategories.map(cat => (
              <button
                ...
                style={{
                  borderBottom: categoryFilter === cat ? `3px solid ${getCategoryColor(cat)}` : 'none'
                }}
              >
                {cat}
              </button>
      ...
      {todos.length > 0 && filteredTodos.length === 0 && (
        <p className="empty-message">
          No {filter} {categoryFilter !== 'all' ? `${categoryFilter} ` : ''}todos.
        </p>
      )}
      </div>
    </div>
